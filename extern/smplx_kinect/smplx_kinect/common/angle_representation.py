...
def mult_aa(a, b):
    a = universe_convert(a, 'aa', 'rotmtx')
    b = universe_convert(b, 'aa', 'rotmtx')
    result = a.dot(b)
    return universe_convert(result, 'rotmtx', 'aa')
...
def rep_shape_forward(orig_shape, src_rep, dst_rep, multiple=False):
    cache = orig_shape, src_rep, dst_rep, multiple
    shape = [-1] + rep2shape(src_rep)
    return shape, cache


def rep_reshape_forward(d, src_rep, dst_rep, multiple=False):
    check_rep(src_rep)
    check_rep(dst_rep)
    orig_shape = d.shape
    shape, cache = rep_shape_forward(orig_shape, src_rep, dst_rep, multiple=multiple)
    return d.reshape(*shape), cache


def rep_shape_backward(cache):
    orig_shape, src_rep, dst_rep, multiple = cache
...
def rep_reshape(shape, src_rep, dst_rep, multiple=False):
    shape, cache = rep_shape_forward(shape, src_rep, dst_rep, multiple=multiple)
...
def convert(d, src_rep, dst_rep, multiple=False):
    check_rep(src_rep)
    check_rep(dst_rep)
    if src_rep == dst_rep:
        return d
    d, cache = rep_reshape_forward(d, src_rep, dst_rep, multiple=multiple)
    d = convert_impl(d, src_rep, dst_rep)
    d = rep_reshape_backward(d, cache)
    return d


def universe_convert(d, src_rep, dst_rep, multiple=False):
    check_rep(src_rep)
    check_rep(dst_rep)
    if src_rep == dst_rep:
        return d
    ...
    d = convert(d, src_rep, dst_rep, multiple=multiple)
